    ...
    if request.method == "POST":
        ...
        with sqlite3.connect("users.db") as con:
            ...
            if listOfTables != []:
                ...
                cur.execute("INSERT into UserInfo (name, username, email, address, password, confirmpassword, playertokens) values (?,?,?,?,?,?,?)", (name, username, email, address,password, confirmpassword, playertokens))
                con.commit()
        flash("Successfully Registered! Log in now.",'error')
...
def hit(message):
    ...
    if (len(RoomUsersArray) == 2):
        ...
        if (sum(playerA.cards) > 21):
            ...
            emit('hitGame', {'msg':RoomUsersArray[1] + " wins! " , 'playerAnewcard': playerAnewcard, 'playerBnewcard': playerBnewcard}, room = room)
            #playerB.score += 1
            ...
            emit('stayGame', {'msg':"Score: " + RoomUsersArray[0] + " <" + str(playerAscore) + "> " + " - v.s. - " +  RoomUsersArray[1] + " <" + str(playerBscore)  + "> " , 'playerAnewcard': playerAnewcard, 'playerBnewcard': playerBnewcard}, room = room)
        ...
        if (sum(playerB.cards) > 21):
            ...
            emit('hitGame', {'msg':RoomUsersArray[0] + " wins! " , 'playerAnewcard': playerAnewcard, 'playerBnewcard': playerBnewcard}, room = room)
            #Update user's score based on their usernames and output it.
            playerAscore = UserScoreTrackerDict.get(RoomUsersArray[0])
            playerBscore = UserScoreTrackerDict.get(RoomUsersArray[1])
            playerAscore += 1
    ...
    if (len(RoomUsersArray) == 2):
        ...
        if (playerA.stay == 1 and playerB.stay == 1):
            ...
            if(sum(playerA.cards) > sum(playerB.cards) and sum(playerA.cards) < 22):
                playerAnewcard = RoomUsersArray[0]  + " wins"
                playerBnewcard = RoomUsersArray[1]  + " loses"
                emit('stayGame', {'msg':RoomUsersArray[0] + " wins! ",'playerAnewcard': playerAnewcard, 'playerBnewcard': playerBnewcard}, room = room)
                #Update user's score based on their usernames and output it.
                playerAscore = UserScoreTrackerDict.get(RoomUsersArray[0])
                playerBscore = UserScoreTrackerDict.get(RoomUsersArray[1])
                playerAscore += 1
        ...
        RoomUsersArray[1] + " has cards: [" + playerBcardsOut + "] count: <" + str(sum(playerB.cards)) + ">\n", 'playerAnewcard': playerAnewcard, 'playerBnewcard': playerBnewcard}, room = room)
            if(sum(playerA.cards) < sum(playerB.cards) and sum(playerA.cards) < 22):
                playerAnewcard = RoomUsersArray[0]  + " loses"
                playerBnewcard = RoomUsersArray[1]  + " wins"
                emit('stayGame', {'msg':RoomUsersArray[1] + " wins! ",'playerBnewcard': playerBnewcard,'playerAnewcard': playerAnewcard}, room = room)
                #Update user's score based on their usernames and output it.
                playerAscore = UserScoreTrackerDict.get(RoomUsersArray[0])
                playerBscore = UserScoreTrackerDict.get(RoomUsersArray[1])
                playerBscore += 1
        ...
        RoomUsersArray[1] + " has cards: [" + playerBcardsOut + "] count: <" + str(sum(playerB.cards)) + ">\n", 'playerAnewcard': playerAnewcard, 'playerBnewcard': playerBnewcard}, room = room)
            if(sum(playerA.cards) == sum(playerB.cards) and sum(playerA.cards) < 22):
                ...
                emit('stayGame', {'msg':RoomUsersArray[1] + " and " + RoomUsersArray[0] + " ties! ",'playerBnewcard': playerBnewcard,'playerAnewcard': playerAnewcard}, room = room)
                #Update user's score based on their usernames and output it.
                playerAscore = UserScoreTrackerDict.get(RoomUsersArray[0])
                playerBscore = UserScoreTrackerDict.get(RoomUsersArray[1])
                emit('stayGame', {'msg':"Score: " + RoomUsersArray[0] + " <" + str(playerAscore) + "> " + " - v.s. - " +  RoomUsersArray[1] + " <" + str(playerBscore)  + "> " , 'playerAnewcard': playerAnewcard, 'playerBnewcard': playerBnewcard}, room = room)
